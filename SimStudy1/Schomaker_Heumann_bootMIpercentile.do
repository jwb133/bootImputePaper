...
VERSION CHANGES:
	...
	CHANGE FROM VERSION 4 TO VERSION 5:	VERSION 4 CONTAINS AN OLDER VERSION OF mim WHICH ASSUMES 2 OR MORE IMPUTATIONS
	                                    VERSION 5 INCLUDES AN UPDATED VERSION OF MATA FUNCTION mim WHICH CAN PROCESS SINGLE IMPUTATION RESULTS. 
	                                    
	CHANGE FROM VERSION 5 TO VERSION 6:	VERSION 5 DOES NOT RECORD THE SKEWNESS OF THE BOOTSTRAP DISTRIBUTION
                                        VERSION 6 RECORDS THE SKEWNESS OF THE BOOTSTRAP DISTRIBUTION
										
	CHANGE FROM VERSION 6 TO VERSION 7: VERSION 6 FITS THE SUBGROUP ANALYSIS MODEL ASSUMING THE SAME VARIANCE PARAMETER FOR MEN AND WOMEN.
	                                    VERSION 7 FITS THE MODEL ONLY TO MEN ALLOWING FOR THE VARIANCE PARAMETER TO DIFFER BETWEEN MEN AND WOMEN
...
ARGS
	...
	- percentObserved : PERCENTAGE OF WEIGHT VALUES OBSERVED
...
if `scenario'== 0 {
	...
	* NAME THE VARIABLE FOR THE ANALYSIS MODEL
	...
	local A_b `"(b_MI_rep[1,1]) (b_MI_rep[1,2]) (b_MI_rep[1,3])"'
	...
	* NAME THE VARIABLE FOR THE ANALYSIS MODEL
	...
	local A_b `"(b_MI_rep[1,1]) (b_MI_rep[1,2]) (b_MI_rep[1,3]) (b_MI_rep[1,4])"'
	...
	* NAME THE VARIABLE FOR THE ANALYSIS MODEL
	...
	local A_b `"(b_MI_rep[1,1]) (b_MI_rep[1,2]) (b_MI_rep[1,3]) (b_MI_rep[1,4]) (b_MI_rep[1,5])"'
...
tempfile file_MI file_bootstrap file_dataset file_incomplete 
 
* SET THE LISTS
local b_list `"b_full b_cca b_MI b_boot bias_boot boot_skew"'
...
matrix boot_skew = J(1,`numberOfAcovars',.)
...
capture log close
log using "Log files\SandH_method4_scenario`scenario'_`sampleSize'obs_observed`percentObserved'_M`M'_B`B'_V`v'", replace
...
forvalues dataset=1(1)`numberOfDatasets' {
	...
	display "SandH_method4: scenario `scenario'; `percentObserved'% observed; B=`B'; M=`M': Processing dataset `dataset'"  
...
quietly {
	...
	* COLLECT THE CCA DATA ESTIMATES	
	...
	timer clear 1
	timer on 1
	* STEP 1: MULTIPLY IMPUTE THE INCOMPLETE DATA BEFORE BOOTSTRAPPING; APPLY RUBIN'S RULES TO GET THE POINT ESTIMATE
	ice weight age height loginsindex sex weightXsex weightMen weightWomen, m(`M') clear ///
		eq(weight:`Icovars') ///
	...
	* STEP 2: BOOTSTRAP THE INCOMPLETE DATASET B TIMES
	forvalues replicate=1(1)`B' {	
		...
		* STEP 3: IMPUTE THE BOOTSTRAPPED DATASET M TIMES
			* CHECKED USING dryrun			
		ice weight age height loginsindex sex weightXsex weightMen weightWomen, m(`M') saving(`file_MI', replace) ///
			eq(weight:`Icovars') ///
			passive(weightXsex:weight*sex\weightMen:weight-weightXsex\weightWomen:weightXsex)
		
		* STEP 4: FIT THE ANALYSIS MODEL USING MIM AND POST THE MI POINT ESTIMATES
		use `file_MI', clear
		forvalues imputation=1(1)`M' {
			if `scenario'<2 regress `Aoutcome' `Acovars' if sex==0 & _mj==`imputation', noconstant
			else regress `Aoutcome' `Acovars' if _mj==`imputation', noconstant
		...
		mata: mim4("b_m", "V_m", `nu_com')
		matrix b_MI_rep = r(coefficients)
	...
	* STEP 5: CALCULATE THE 3 TYPES OF BOOTSTRAPPED CONFIDENCE INTERVALS
	...
	* CALCULATE THE SKEWNESS AROUND THE MEAN OF THE BOOTSTRAP DISTRIBUTION
	local column 0
	foreach covariate of local Acovars {
		local column = `column'+1
		quietly summarize `covariate', detail
		matrix boot_skew[1,`column'] = r(skewness)  
	}
...
if c(version) >=13 saveold "Results\SandH_method4_scenario`scenario'_`sampleSize'obs_observed`percentObserved'_M`M'_B`B'_V`v'", version(12) replace
else save "Results\SandH_method4_scenario`scenario'_`sampleSize'obs_observed`percentObserved'_M`M'_B`B'_V`v'", replace
